...
const SettingInternal = () => {
  ...
  const columnsInternal = [
    ...
    },
    {
      title: "Internal",
      sorter: true,
      render: (item) => <label className="text-capitalize">{item.name}</label>,
    },
    {
      title: "Status",
      render: (item) => (
  ...
  const saveDataForm = async () => {
    try {
      ...
      await ApiPostRequest(`setting`, formDataInternal);
      PnomNotification({
        type: "success",
        message: "Berhasil disimpan!",
        description: "Data internal berhasil disimpan!",
      });
      getDataInternal();
    } catch (error) {
      notificationError(error);
          ...
          <Form>
            <Row gutter={[24, 0]}>
              <Col md={{ span: 24 }} xs={{ span: 24 }}>
                <Form.Item className="username mb-0" label="Internal">
                  <Input
                    value={formData.value}
                    onChange={(e) =>
              ...
              <Col md={{ span: 24 }} xs={{ span: 24 }}>
                <Form.Item className="username mb-0" label="Tipe">
                  <Input
                    value={formData.name}
                    onChange={(e) =>
...
export default SettingInternal;
