...
const ProductCreatePage = ({
        ...
        {dataImages.images.map((item, index) => (
          <Col xs={24}>
            {item.imageThumb === undefined ? (
              ...
              <Button
                onClick={() => handleDeleteImage(dataImages, index)}
                className="trash"
              >
                <DeleteOutlined />
              </Button>
  ...
  const handleUploadImageVariant = async (event, index) => {
    try {
      ...
      if (newDataSales[index].images.length > 1) {
        setUpdateImage(true);
        await deleteImageDetail(
          uuidData,
          newDataSales[index].uuid,
          newDataSales[index].images[index].uuid,
        );
  ...
  const deleteImageDetail = async (uuid, uuidDetail, uuidDetailImage) => {
    try {
      await ApiDeleteRequest(
        `product/item/${uuid}/detail/${uuidDetail}/image/${uuidDetailImage}`,
      );
    } catch (error) {
      notificationError(error);
    }
  ...
  const submitDataProduct = async () => {
    try {
      ...
      if (updateImageCover === true)
        formData.imageCover = dataProduct.imageCover;
  ...
  return (
    <>
      <div className="create-product">
        <Row gutter={[24, 0]}>
          <Col xs={24} xl={24}>
            <Card bordered={false} className="criclebox mb-14">
                ...
                <Form>
                  <Row className="mb-2" gutter={[24, 0]}>
                    ...
                    >
                      ...
                      <Form.Item className="username mb-0" label="Nama Produk">
                        <Input
                          maxLength={255}
                      ...
                      <Form.Item className="username mb-0" label="Kategori">
                        <Select
                          showSearch
                      ...
                      <Form.Item className="username mb-0" label="Rekomendasi">
                        <Select
                          showSearch
                          ...
                          >
                            ...
                            >
                              <input
                                ...
                                onChange={(event) =>
                                  handleUploadImageVariant(event, index)
                                }
                                accept="image/*"
                              />
                            </Form.Item>

                            {item.images.length > 0 ? componentImage(item) : ""}
                          </Col>
                        </Row>
                      ))}
                    </Col>
